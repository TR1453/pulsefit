...
function parseOptionalHeight(value) {
  const parsed = parseOptionalNumber(value);
  return parsed ? Number(parsed.toFixed(1)) : null;
}
...
function openSettingsDrawer() {
  if (currentProfile) {
    ...
    updateSettingsMeta(currentProfile);
  }
...
function buildProfileMetadata() {
  ...
  const height = parseOptionalHeight(metadata.height_cm);
  ...
  return {
    ...
    height_cm: height && height >= 100 && height <= 250 ? height : null,
  };
}
...
function fillProfileForm(profile) {
  ...
  profileForm.height.value = profile?.height_cm ?? currentUser?.user_metadata?.height_cm ?? "";
...
function buildUserSummary(entries, profile) {
  ...
  return {
    ...
    heightCm: profile?.height_cm || currentUser?.user_metadata?.height_cm || null,
...
function buildOpenAIContext(entries, profile) {
  ...
  return [
    ...
    `Groesse: ${summary.heightCm ? `${summary.heightCm} cm` : "nicht gesetzt"}`,
...
async function ensureProfileRecord() {
  ...
  if (!profile) {
    const initialProfile = {
      ...
      height_cm: metadataProfile.height_cm,
      current_weight: null,
      target_weight: null,
  ...
  const shouldPatchFromMetadata =
    ...
    (!profile.height_cm && metadataProfile.height_cm);
  ...
  if (shouldPatchFromMetadata) {
    const mergedProfile = {
      ...
      height_cm: profile.height_cm || metadataProfile.height_cm,
    };
...
function updateSettingsMeta(profile) {
  settingsEmail.textContent = currentUser?.email || "-";
  settingsRole.textContent = isBossUser() ? "Chef" : "Mitglied";

  if (!profile?.first_name && !profile?.last_name && currentUser?.email) {
    settingsRole.textContent = isBossUser() ? "Chef" : "Mitglied";
  }
}
...
registerForm.addEventListener("submit", async (event) => {
  ...
  const height = parseOptionalHeight(registerForm.height.value);
  const email = registerForm.email.value.trim();
  const password = registerForm.password.value;

  if (!firstName || !lastName || !age || !height || !email || !password) {
  ...
  if (height < 100 || height > 250) {
    showMessage(registerMessage, "Bitte gib eine realistische Groesse zwischen 100 und 250 cm ein.", "error");
    return;
  }
...
profileForm.addEventListener("submit", async (event) => {
  ...
  const height = parseOptionalHeight(profileForm.height.value);
  ...
  if (profileForm.height.value && (!height || height < 100 || height > 250)) {
    showMessage(profileMessage, "Bitte gib eine realistische Groesse zwischen 100 und 250 cm ein.", "error");
    return;
  }
...
passwordForm.addEventListener("submit", async (event) => {
  ...
  if (!currentUser) {
    showMessage(passwordMessage, "Bitte melde dich zuerst an.", "error");
    return;
  }
  ...
  if (!newPassword || !confirmPassword) {
    showMessage(passwordMessage, "Bitte fuelle beide Passwort-Felder aus.", "error");
    return;
  }

  if (newPassword !== confirmPassword) {
    showMessage(passwordMessage, "Die Passwoerter stimmen nicht ueberein.", "error");
    return;
  }
  ...
  if (passwordValidationMessage) {
    showMessage(passwordMessage, passwordValidationMessage, "error");
    return;
  }
  ...
  try {
    ...
    passwordForm.reset();
    showMessage(passwordMessage, "Passwort erfolgreich aktualisiert.", "success");
  } catch (error) {
    console.error(error);
    showMessage(passwordMessage, "Passwort konnte nicht aktualisiert werden.", "error");
  }
});
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && !settingsDrawer.classList.contains("hidden")) {
    closeSettingsDrawer();
  }
});
